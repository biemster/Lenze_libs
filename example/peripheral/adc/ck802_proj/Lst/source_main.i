...
# 24 "../../../../components/driver/include/spif.h"
...
# 43 "../../../../components/driver/include/otp.h"
...
# 45 "../source/main.c" 2
# 1 "../../../../components/driver/include/watchdog.h" 1
# 33 "../../../../components/driver/include/watchdog.h"
#define __WATCHDOG_ROM_H__ 
# 42 "../../../../components/driver/include/watchdog.h"
typedef enum
{
    WDG_2S = 0,
    WDG_4S = 1,
...
# 81 "../../../../components/driver/include/watchdog.h"

# 81 "../../../../components/driver/include/watchdog.h" 3 4
_Bool 
# 81 "../../../../components/driver/include/watchdog.h"
    _symrom_watchdog_init(WDG_CYCLE_Type_e cycle,
# 81 "../../../../components/driver/include/watchdog.h" 3 4
                                         _Bool 
# 81 "../../../../components/driver/include/watchdog.h"
                                              int_mode);
...
uint8 g_pConnectionBuffer[(1 * ( (2 * (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2))*2) +(2 * (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2))) + (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2)) ))] __attribute__ ((aligned(4)));
llConnState_t pConnContext[1];
...
uint8 g_largeHeap[(2*1024)] __attribute__ ((aligned(4)));
...
static void hal_low_power_io_init(void)
{
    ...
    for(uint8_t i=0; i<sizeof(ioInit)/sizeof(ioinit_cfg_t); i++)
        _symrom_gpio_pull_set(ioInit[i].pin,ioInit[i].type);

    {(*(volatile unsigned int *)(0x4000f014)=(unsigned int)(((*(volatile unsigned int *)(0x4000f014))& (~((((unsigned int)1<<((22)-(21)+1))-1)<<(21))))| ((unsigned int)((0x03&(0x01)))<<(21))));__asm volatile("nop");};
    _symrom_pwrmgr_RAM_retention_set();
    _symrom_pwrmgr_LowCurrentLdo_enable();
}

static void ble_mem_init_config(void)
{
    _symrom_osal_mem_set_heap((osalMemHdr_t*)g_largeHeap, (2*1024));
    _symrom_LL_InitConnectContext(pConnContext,
                          g_pConnectionBuffer,
                          1,
                          2,
                          2,
...
static void hal_rfphy_init(void)
{
    ...
    _symrom_g_rfPhyTxPower = 0x0a ;

    _symrom_g_rfPhyPktFmt = 1;

    _symrom_g_rfPhyFreqOffSet =0;

    {(*(volatile unsigned int *)(0x4000f0bc)=(unsigned int)(((*(volatile unsigned int *)(0x4000f0bc))& (~((((unsigned int)1<<((4)-(0)+1))-1)<<(0))))| ((unsigned int)((0x1f&(0x09)))<<(0))));__asm volatile("nop");};
    {(*(volatile unsigned int *)(0x4000f0bc)=(unsigned int)(((*(volatile unsigned int *)(0x4000f0bc))& (~((((unsigned int)1<<((6)-(5)+1))-1)<<(5))))| ((unsigned int)((0x03&(0x01)))<<(5))));__asm volatile("nop");};
    hal_rom_boot_init();
    csi_vic_set_prio((IRQn_Type)BB_IRQn, 0);
    csi_vic_set_prio((IRQn_Type)TIM1_IRQn, 1);
    csi_vic_set_prio((IRQn_Type)TIM2_IRQn, 1);
...
static void hal_init(void)
{
    ...
    _symrom_clk_init(_symrom_g_system_clk);
...
int main(void)
{
    ...
    _symrom_g_system_clk = SYS_CLK_XTAL_16M;
    g_clk32K_config = CLK_32K_RCOSC;

    _symrom_drv_irq_init();
    init_config();
    hal_rfphy_init();
    hal_init();

    if(_symrom_gpio_read(P20)==1)
 {

 }

    dbg_printf("SDK Version ID %08x \n",((5<<16)|(0<<8)|(0)));
    dbg_printf("rfClk %d rcClk %d sysClk %d tpCap[%02x %02x]\n",_symrom_g_rfPhyClkSel,g_clk32K_config,_symrom_g_system_clk,_symrom_g_rfPhyTpCal0,_symrom_g_rfPhyTpCal1);
    dbg_printf("sizeof(struct ll_pkt_desc) = %d, buf size = %d\n", sizeof(struct ll_pkt_desc), (1 * ( (2 * (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2))*2) +(2 * (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2))) + (((0 == 1) ? 1 : 0) * 262 + ((0 == 0) ? 1 : 0) * 42 + (sizeof(struct ll_pkt_desc) - 2)) )));
    dbg_printf("sizeof(g_pConnectionBuffer) = %d, sizeof(pConnContext) = %d, sizeof(largeHeap)=%d \n", sizeof(g_pConnectionBuffer), sizeof(pConnContext),sizeof(g_largeHeap))
                                                                              ;
    dbg_printf("[REST CAUSE] %d\n ",_symrom_g_system_reset_cause);
    app_main();
}
