...
const uint8_t c_gpio_index[GPIO_NUM] = \
	{0,1,2,3,7,9,10,11,14,15,16,17,18,20,23,24,25,26,27,31,32,33,34};
		
static int hal_gpio_interrupt_disable(gpio_pin_e pin)
{
  //enable gpio 15 gpio falling edge intteruput
  //uint32_t gpio_tmp = AP_GPIO->inttype_level;
...
void hal_gpio_fmux_control(gpio_pin_e pin, bit_action_e value)
{
    if(value)			
...
{
    hal_gpio_fmux_control(pin,Bit_DISABLE);
    if(type == GPIO_OUTPUT)
    {
        AP_GPIO->swporta_dr &= ~BIT(pin);
...
void hal_gpio_wakeup_src(uint32_t* p34_32,uint32_t* p31_P00)
{
    *p31_P00 = AP_AON->GPIO_WAKEUP_SRC[0];
    *p34_32 = AP_AON->GPIO_WAKEUP_SRC[1];
}

void hal_gpio_pin2pin3(gpio_pin_e pin, uint8_t en) 
{
    if(en) 
...
{	    
    ...
    hal_gpio_fmux_control(pin, Bit_ENABLE);
    reg_index = pin / 4;
    bit_index = pin % 4;
    l = 8 * bit_index;
    h = l + 5;
...
void hal_gpio_fmux_default(gpio_pin_e pin)
{	    
    uint8_t h = 0,l = 0;
    uint32_t reg_index;
    uint32_t bit_index;
    ...
    l = 8 * bit_index;
    h = l + 5;
    subWriteReg(&(AP_IOMUX->gpio_sel[reg_index]),h,l,0);
}
