...
typedef struct{	       
	__IO uint32_t    GPIO_WAKEUP_SRC0;
	__IO uint32_t    GPIO_WAKEUP_SRC1;
	__IO uint32_t    PCLK_CLK_GATE_EN;
	__IO uint32_t    XTAL_16M_CTRL;
	__IO uint32_t    SLEEP_R0;
	__IO uint32_t    SLEEP_R1;
	__IO uint32_t    SLEEP_R2;    
	__IO uint32_t    SLEEP_R3;
}AP_EFUSE_TypeDef;
...
typedef struct{
	__IO uint32_t HCD_INT_STS;
	__IO uint32_t HCD_INT_EN;
	__IO uint32_t HCD_STS;
	__IO uint32_t HCD_CTL;
	__IO uint32_t HCD_FIFO_SZ;
	__IO uint32_t HCD_THD;
	__IO uint32_t HCD_OUT_SZ;
}AP_USB_HCD_GLB_TypeDef;
...
typedef struct{
	...
	__I  uint32_t RSVD40[5];
	__IO uint32_t PORTSTSCTL;
	__I  uint32_t RSVD54[14];
	__IO uint32_t TOKEN;
	__IO uint32_t OTGCTLSTS;
}AP_USB_HCD_MAC_TypeDef;

typedef struct{
	__IO uint32_t DEV_IN_EP_DMA[MAX_EP_IP_SUPPORT];
	__IO uint32_t DEV_OUT_EP_DMA[MAX_EP_IP_SUPPORT];
	__IO uint32_t HCD_IN_EP_DMA;
	__IO uint32_t HCD_OUT_EP_DMA;
	__IO uint32_t HCD_IN_TOKEN;
	__IO uint32_t HCD_OUT_TOKEN;
	__IO uint32_t HCD_IN_MAX_PKT;
	__IO uint32_t HCD_OUT_MAX_PKT;
}AP_USB_DIU_TypeDef;
...
#define SPIF_BASE_ADDR       (0x11000000UL)           /*spif*/
...
#define AP_CACHE_BASE        (AP_APB0_BASE + 0x3040)
#define AP_IOMUX_BASE        (AP_APB0_BASE + 0x3800)/*iomux*/
#define AP_UART0_BASE        (AP_APB0_BASE + 0x4000)/*uart*/
...
/*
//#define SRAM3_BASE_ADDRESS     0x20010000//todo
...
//#define AP_RNG             ((AP_RNG_TypeDef *) AP_PWM_BASE)
...
#define AP_EFUSE             ((AP_EFUSE_TypeDef  *) AP_EFUSE_BASE)
