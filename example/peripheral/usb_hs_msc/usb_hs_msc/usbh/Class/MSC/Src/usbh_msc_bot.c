...
USBH_StatusTypeDef USBH_MSC_BOT_Process(USBH_HandleTypeDef *phost, uint8_t lun)
{
  ...
  {
  case BOT_SEND_CBW:
    //USBH_UsrLog("BOT_SEND_CBW\n");
    MSC_Handle->hbot.cbw.field.LUN = lun;
    MSC_Handle->hbot.state = BOT_SEND_CBW_WAIT;
    USBH_BulkSendData(phost, MSC_Handle->hbot.cbw.data,
                      BOT_CBW_LENGTH, MSC_Handle->OutEp);
    break;

  case BOT_SEND_CBW_WAIT:
    //USBH_UsrLog("BOT_SEND_CBW_WAIT\n");
    URB_Status = USBH_LL_GetURBState(phost, MSC_Handle->OutEp);
  ...
  case BOT_DATA_IN:
    /* Send first packet */
    //USBH_UsrLog("BOT_DATA_IN\n");
    USBH_BulkReceiveData(phost, MSC_Handle->hbot.pbuf,
                         MSC_Handle->InEpSize, MSC_Handle->InEp);
  ...
  case BOT_DATA_IN_WAIT:
    //USBH_UsrLog("BOT_DATA_IN_WAIT\n");
    URB_Status = USBH_LL_GetURBState(phost, MSC_Handle->InEp);
    if (URB_Status == USBH_URB_DONE)
    {
  ...
  case BOT_DATA_OUT:
    ...
    //USBH_UsrLog("BOT_DATA_OUT\n");
    USBH_BulkSendData(phost, MSC_Handle->hbot.pbuf,
                      MSC_Handle->OutEpSize, MSC_Handle->OutEp);
  ...
  case BOT_RECEIVE_CSW:
    ...
    //USBH_UsrLog("BOT_RECEIVE_CSW\n");
    USBH_BulkReceiveData(phost, MSC_Handle->hbot.csw.data,
                         BOT_CSW_LENGTH, MSC_Handle->InEp);
  ...
  case BOT_RECEIVE_CSW_WAIT:
    ...
    //USBH_UsrLog("BOT_RECEIVE_CSW_WAIT\n");
    URB_Status = USBH_LL_GetURBState(phost, MSC_Handle->InEp);
  ...
  case BOT_ERROR_OUT:
    ...
    if (error == USBH_OK)
    {
      ...
      toggle = USBH_LL_GetToggle(phost, MSC_Handle->OutEp);
      USBH_LL_SetToggle(phost, MSC_Handle->OutEp, 1U - toggle);
  ...
  case BOT_UNRECOVERED_ERROR:
    USBH_UsrLog("BOT_UNRECOVERED_ERROR\n");
...
static BOT_CSWStatusTypeDef USBH_MSC_DecodeCSW(USBH_HandleTypeDef *phost)
{
  ...
  if (USBH_LL_GetLastXferSize(phost, MSC_Handle->InEp) != BOT_CSW_LENGTH)
  {
