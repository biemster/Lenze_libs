# 1 "../../../../components/profiles/GATT/gattservapp.c"
...
# 1 "../../../../components/profiles/GATT/gattservapp.c"
# 47 "../../../../components/profiles/GATT/gattservapp.c"
...
# 48 "../../../../components/profiles/GATT/gattservapp.c" 2
...
# 49 "../../../../components/profiles/GATT/gattservapp.c" 2
...
# 59 "../../../../components/ble/include/gatt.h" 2
...
# 51 "../../../../components/profiles/GATT/gattservapp.c" 2
...
# 52 "../../../../components/profiles/GATT/gattservapp.c" 2
...
# 53 "../../../../components/profiles/GATT/gattservapp.c" 2
...
# 78 "../../../../components/profiles/GATT/gattservapp.c"
typedef struct
{
  uint16 handle;
...
# 105 "../../../../components/profiles/GATT/gattservapp.c"
uint8 GATTServApp_TaskID;

uint8 appTaskID = 0xFF;


static serviceCBsList_t *serviceCBsList = 0;


static uint8 attrLen;
static uint8 attrValue[247 -1];
static attMsg_t rsp;
...
static const gattAttrType_t gattService = { 2, _symrom_gattServiceUUID };


static uint8 serviceChangedCharProps = 0x20;





static uint16 serviceChange_indCharCfg = 0x0000;
...
# 199 "../../../../components/profiles/GATT/gattservapp.c"
...
# 224 "../../../../components/profiles/GATT/gattservapp.c"
void GATTServApp_RegisterForMsg( uint8 taskID )
{
 appTaskID = taskID;
}
# 238 "../../../../components/profiles/GATT/gattservapp.c"
void GATTServApp_Init( uint8 taskId )
{
 GATTServApp_TaskID = taskId;


 _symrom_GATT_RegisterForReq( GATTServApp_TaskID );
}
# 259 "../../../../components/profiles/GATT/gattservapp.c"
uint16 GATTServApp_ProcessEvent( uint8 task_id, uint16 events )
{
  if ( events & 0x8000 )
  {
    ...
    if ( (pMsg = ( osal_event_hdr_t *)_symrom_osal_msg_receive( GATTServApp_TaskID )) != 0 )
    {
      ...
      _symrom_osal_msg_deallocate( (uint8 *)pMsg );
    }
...
# 307 "../../../../components/profiles/GATT/gattservapp.c"
                                       ...
                                       const gattServiceCBs_t *pServiceCBs )
{
  ...
  if ( pAttrs != 0 )
  {
    ...
    status = _symrom_GATT_RegisterService( &service );
    if ( ( status == 0x00 ) && ( pServiceCBs != 0 ) )
    {

      status = gattServApp_RegisterServiceCBs( ( (pAttrs)[0].handle ),
                                               pServiceCBs );
    }
  }
  else
...
# 349 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t GATTServApp_AddService( )
{
 uint8 status = 0x00;

 status = GATTServApp_RegisterService( gattAttrTbl, ( sizeof( gattAttrTbl ) / sizeof( gattAttribute_t ) ),
                                 &gattServiceCBs );
 return ( status );
}
# 370 "../../../../components/profiles/GATT/gattservapp.c"
                                                 ...
                                                 const gattServiceCBs_t *pServiceCBs )
{
  ...
  pNewItem = (serviceCBsList_t *)_symrom_osal_mem_alloc( sizeof( serviceCBsList_t ) );
  if ( pNewItem == 0 )
  {
...
# 426 "../../../../components/profiles/GATT/gattservapp.c"
const gattServiceCBs_t *gattServApp_FindServiceCBs( uint16 handle )
{
  ...
  return ( (gattServiceCBs_t *)0 );
}
# 453 "../../../../components/profiles/GATT/gattservapp.c"
void gattServApp_ProcessMsg( gattMsgEvent_t *pMsg )
{
 uint16 errHandle = 0x0000;
 uint8 status;
# 466 "../../../../components/profiles/GATT/gattservapp.c"
 switch ( pMsg->method )
 {
  case 0x02:
  ...
  if ( status != 0x00 )
  {
    ...
    if ( pMsg->hdr.status != 0x14 )
    {
      ...
      _symrom_ATT_ErrorRsp( pMsg->connHandle, pRsp );
    }
  }
}
# 530 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessExchangeMTUReq( gattMsgEvent_t *pMsg )
{
 attExchangeMTURsp_t *pRsp = &rsp.exchangeMTURsp;
# 541 "../../../../components/profiles/GATT/gattservapp.c"
 pRsp->serverRxMTU = _symrom_g_ATT_MTU_SIZE_MAX;


 _symrom_ATT_ExchangeMTURsp( pMsg->connHandle, pRsp );

 return ( 0x00 );
}
# 560 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessFindByTypeValueReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
  ...
  _symrom_osal_memset( pRsp, 0, sizeof( attFindByTypeValueRsp_t ) );
# 577 "../../../../components/profiles/GATT/gattservapp.c"
  pAttr = _symrom_GATT_FindHandleUUID( pReq->startHandle, pReq->endHandle,
                               pReq->type.uuid, pReq->type.len, &service );

  while ( ( pAttr != 0 ) && ( pRsp->numInfo < _symrom_g_ATT_MAX_NUM_HANDLES_INFO ) )
  {
    uint16 grpEndHandle;
    ...
    if ( GATTServApp_ReadAttr( pMsg->connHandle, pAttr, service, attrValue,
                               &attrLen, 0, (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-7) ) == 0x00 )
    {

      if ( ( pReq->len == attrLen ) && _symrom_osal_memcmp( pReq->value, attrValue, attrLen) )
      {
    ...
    pAttr = _symrom_GATT_FindNextAttr( pAttr, pReq->endHandle, service, &grpEndHandle );
    ...
    if ( pRsp->handlesInfo[pRsp->numInfo].handle != 0 )
    {
      ...
      else
      {
        ...
        pRsp->handlesInfo[pRsp->numInfo++].grpEndHandle = 0xFFFF;
      }
    }
  }
  ...
  if ( pRsp->numInfo > 0 )
  {
    ...
    _symrom_ATT_FindByTypeValueRsp( pMsg->connHandle, pRsp );
...
# 647 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessReadByTypeReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
  ...
  while ( dataLen <= (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-4) )
  {
    uint16 service;
    gattAttribute_t *pAttr;
    ...
    pAttr = _symrom_GATT_FindHandleUUID( startHandle, pReq->endHandle, pReq->type.uuid,
...
# 689 "../../../../components/profiles/GATT/gattservapp.c"
    status = GATTServApp_ReadAttr( pMsg->connHandle, pAttr, service, attrValue,
                                   &attrLen, 0, (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-4) );
    if ( status != 0x00 )
    {
      break;
    }
    ...
    if ( dataLen + attrLen > (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-4) )
    {
      break;
    }


    pRsp->dataList[dataLen++] = ((pAttr->handle) & 0xFF);
    pRsp->dataList[dataLen++] = (((pAttr->handle) >> 8) & 0xFF);

    _symrom_osal_memcpy( &(pRsp->dataList[dataLen]), attrValue, attrLen );
    dataLen += attrLen;
  ...
  if ( dataLen > 0 )
  {
    ...
    _symrom_ATT_ReadByTypeRsp( pMsg->connHandle, pRsp );
...
# 767 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessReadReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
 ...
 pAttr = _symrom_GATT_FindHandle( pReq->handle, &service );
 if ( pAttr != 0 )
 {
  ...
  status = GATTServApp_ReadAttr( pMsg->connHandle, pAttr, service, pRsp->value,
                                 &pRsp->len, 0, (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-1) );
  if ( status == 0x00 )
  {

   _symrom_ATT_ReadRsp( pMsg->connHandle, pRsp );
  }
 }
  else
  {
...
# 819 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessReadBlobReq( gattMsgEvent_t* pMsg, uint16* pErrHandle )
{
    ...
    if ( pAttr != 0 )
    {
        ...
        status = GATTServApp_ReadAttr( pMsg->connHandle, pAttr, service, pRsp->value,
                                       &pRsp->len, pReq->offset, (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-1) );

        if ( status == 0x00 )
        {

            _symrom_ATT_ReadBlobRsp( pMsg->connHandle, pRsp );
        }
    }
    else
    {
...
# 866 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessReadByGrpTypeReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
  ...
  uint8 dataLen = 0;
  uint8 status = 0x00;
# 882 "../../../../components/profiles/GATT/gattservapp.c"
  pAttr = _symrom_GATT_FindHandleUUID( pReq->startHandle, pReq->endHandle,
                               pReq->type.uuid, pReq->type.len, &service );
  while ( pAttr != 0 )
  {
    uint16 endGrpHandle;
# 903 "../../../../components/profiles/GATT/gattservapp.c"
    status = GATTServApp_ReadAttr( pMsg->connHandle, pAttr, service, attrValue,
                                   &attrLen, 0, (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-6) );
    if ( status != 0x00 )
    {
    ...
    else
    {
      ...
      if ( dataLen + attrLen > (_symrom_ATT_GetCurrentMTUSize(pMsg->connHandle)-6) )
      {
        break;
      }
    }
    ...
    pAttr = _symrom_GATT_FindNextAttr( pAttr, pReq->endHandle, service, &endGrpHandle );
    ...
    if ( pAttr != 0 )
    {
      ...
      pRsp->dataList[dataLen++] = ((endGrpHandle) & 0xFF);
      pRsp->dataList[dataLen++] = (((endGrpHandle) >> 8) & 0xFF);
    }
    else
    {

      pRsp->dataList[dataLen++] = ((0xFFFF) & 0xFF);
      pRsp->dataList[dataLen++] = (((0xFFFF) >> 8) & 0xFF);
    }
  ...
  if ( dataLen > 0 )
  {
    ...
    _symrom_ATT_ReadByGrpTypeRsp( pMsg->connHandle, pRsp );
...
# 995 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_ProcessWriteReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
  ...
  if ( pAttr != 0 )
  {
# 1026 "../../../../components/profiles/GATT/gattservapp.c"
    if ( status == 0x00 )
    {

      status = GATTServApp_WriteAttr( pMsg->connHandle, pReq->handle,
                                      pReq->value, pReq->len, 0 );
      if ( ( status == 0x00 ) && ( pReq->cmd == 0 ) )
      {

        _symrom_ATT_WriteRsp( pMsg->connHandle );
      }
    }
  }
  else
  ...
  return ( pReq->cmd ? 0x00 : status );
}
# 1060 "../../../../components/profiles/GATT/gattservapp.c"
pfnGATTReadAttrCB_t gattServApp_FindReadAttrCB( uint16 handle )
{
  const gattServiceCBs_t *pCBs = gattServApp_FindServiceCBs( handle );

  return ( ( pCBs == 0 ) ? 0 : pCBs->pfnReadAttrCB );
}
# 1076 "../../../../components/profiles/GATT/gattservapp.c"
pfnGATTWriteAttrCB_t gattServApp_FindWriteAttrCB( uint16 handle )
{
  const gattServiceCBs_t *pCBs = gattServApp_FindServiceCBs( handle );

  return ( ( pCBs == 0 ) ? 0 : pCBs->pfnWriteAttrCB );
}
# 1115 "../../../../components/profiles/GATT/gattservapp.c"
bStatus_t gattServApp_WriteAttrCB( uint16 connHandle, gattAttribute_t *pAttr,
                                          uint8 *pValue, uint8 len, uint16 offset )
{
 ...
 if ( pAttr->type.len == 2 )
 {
  ...
  {
   case 0x2902:
    serviceChange_indCharCfg = ((uint16)(((pValue[0]) & 0x00FF) + (((pValue[1]) & 0x00FF) << 8)));
   break;
...
# 1172 "../../../../components/profiles/GATT/gattservapp.c"
                            ...
                            uint16 offset, uint8 maxLen )
{
 uint8 useCB = 0;
 bStatus_t status = 0x00;
# 1206 "../../../../components/profiles/GATT/gattservapp.c"
 if ( pAttr->type.len == 2 )
 {
  ...
  switch ( uuid )
  {
   ...
   case 0x2801:
   {
     ...
     *pLen = pType->len;
     _symrom_osal_memcpy( pValue, pType->uuid, pType->len );
    }
    else
    {
     status = 0x0b;
   ...
   case 0x2803:
   {
     ...
     *pLen = 1;
     ...
     pCharValue = _symrom_GATT_FindHandle( pAttr->handle+1, 0 );
     if ( pCharValue != 0 )
     {
      ...
      *pLen += (2 + pCharValue->type.len);
      ...
      pValue[1] = ((pCharValue->handle) & 0xFF);
      pValue[2] = (((pCharValue->handle) >> 8) & 0xFF);


      _symrom_osal_memcpy( &(pValue[3]), pCharValue->type.uuid, pCharValue->type.len );
     }
     else
     {
      ...
      *pLen += (2 + 2);
      ...
      _symrom_osal_memset( &(pValue[1]), 0, (2 + 2) );
     }
    }
    else
    {
   ...
   break;
# 1329 "../../../../components/profiles/GATT/gattservapp.c"
   default:
   {
    useCB = 1;
...
# 1374 "../../../../components/profiles/GATT/gattservapp.c"
                             ...
                             uint8 *pValue, uint16 len, uint16 offset )
{
  ...
  pAttr = _symrom_GATT_FindHandle( handle, &service );
  if ( pAttr != 0 )
  {
