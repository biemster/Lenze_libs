...
# 56 "../../../components/ble/controller/rf_phy_driver.h"
# 1 "../../../components/ble/controller/ll_hw_drv.h" 1
...
extern uint32_t _symrom_g_smartWindowSize ;
extern volatile sysclk_t _symrom_g_system_clk;
# 70 "../src_patch/patch.c"
uint32* pGlobal_config;


const uint32 s_macaddr[2];
# 84 "../src_patch/patch.c"
void __wdt_init(void)
{
    typedef void (*my_function)(void );
    my_function pFunc = 0;
    pFunc = (my_function)(((uint32_t)((*(uint16 *)(0x1fff0000 + (127 << 1))) + (0x1fff0000&0xffff0000))));

    if (pFunc != ((my_function)0x1fff0000))
        pFunc();
}
...
{
    __wdt_init();
    _symrom_wakeup_init0();
}

void ll_hw_go1(void)
{
 _symrom_ll_hw_go0();
 _symrom_gpio_write(P23, 1);
 _symrom_gpio_write(P23, 0);
}
...
uint8_t ll_hw_write_tfifo1(uint8_t* txPkt, uint16_t pktLen)
{
 ...
 _symrom_ll_hw_get_tfifo_info(&rdPtr,&wrPtr,&wrDepth);
 uint16_t tfifoSpace = 0x07ff & (*(uint32_t *) (0x40031000 + 0x74));


 if((pktLen>0) && (wrDepth+(pktLen>>2)<tfifoSpace-(10))){
   ...
   *(volatile uint32_t *)((0x40031000 +0x400)) = *p_txPkt++;
  }




  int rduLen = pktLen&0x03;

  if( rduLen==3){
   *(volatile uint32_t *)((0x40031000 +0x400)) = *(uint16_t *)(txPkt+pktLen-3) | (txPkt[pktLen-1]<<16) ;
  }else if( rduLen==2){
   *(volatile uint32_t *)((0x40031000 +0x400)) = *(uint16_t *)(txPkt+pktLen-2);
  }else if( rduLen==1){
   *(volatile uint32_t *)((0x40031000 +0x400)) = *(txPkt+pktLen-1);
  }else{
   *(volatile uint32_t *)((0x40031000 +0x400)) = *p_txPkt;
  }
...
uint16 ll_generateTxBuffer1(int txFifo_vacancy, uint16 *pSave_ptr)
{
 ...
 llConnState_t *connPtr;
    ...
    connPtr = &_symrom_conn_param[0];


    if(connPtr->llMode == 0x0032
      || connPtr->llMode == 0x0022)
    {
        *(volatile uint32_t *)((0x40031000 +0x400)) = 0x00000001;

        connPtr->ll_buf.tx_not_ack_pkt->valid = 0;
        tx_num ++;
    }

    else if (connPtr->ll_buf.tx_not_ack_pkt->valid != 0)
    {
    ...
    *pSave_ptr = _symrom_ll_hw_get_tfifo_wrptr();
  ...
  && txFifo_vacancy >= connPtr->ll_buf.ntrm_cnt)
 {
  ...
  connPtr->ll_buf.ntrm_cnt = 0;
    ...
    _symrom_rfCounters.numTxCtrl = 0;


    if ((connPtr->ll_buf.tx_not_ack_pkt->valid == 0 ||
        (connPtr->ll_buf.tx_not_ack_pkt->header & 0x3) != 3)
         && connPtr->ctrlDataIsPending
         ...
         && txFifo_vacancy > connPtr->ll_buf.ntrm_cnt)
    {
        ...
        _symrom_rfCounters.numTxCtrl = 1;
    }
...
void init_config(void)
{
 ...
 pGlobal_config[26] = (uint32_t)&__initial_sp;


 pGlobal_config[11] = 0x00000004 | 0x00000002 | 0x00000001
 ...
 pGlobal_config[20] = 10;


 pGlobal_config[14] = _symrom_g_system_clk;
# 287 "../src_patch/patch.c"
 pGlobal_config[7] = 1850;

 if(_symrom_g_system_clk==SYS_CLK_XTAL_16M)
 {
  pGlobal_config[8] = 16;
 }
 else if(_symrom_g_system_clk==SYS_CLK_DBL_32M)
 {
  pGlobal_config[8] = 18;
 }
 else if(_symrom_g_system_clk==SYS_CLK_DLL_48M)
 {
  pGlobal_config[8] = 20;
 }
 else if(_symrom_g_system_clk==SYS_CLK_DLL_64M)
 {
  pGlobal_config[8] = 24;
 }


 pGlobal_config[5] = 1500000;
 pGlobal_config[6] = 1500;

 pGlobal_config[27] = 60;


 pGlobal_config[15] = 54;
 pGlobal_config[16] = 8;
 pGlobal_config[17] = 40;




 pGlobal_config[25] = 58;
 pGlobal_config[24] = 49;



 pGlobal_config[32] = 59;
 pGlobal_config[33] = 8;
 pGlobal_config[34] = 40;



 pGlobal_config[36] = 70;
 pGlobal_config[35] = 57;



 pGlobal_config[28] = 90;
 pGlobal_config[29] = 8;
 pGlobal_config[30] = 60;


 pGlobal_config[0] = 1400;
 pGlobal_config[12] = 1400;
 ...
 pGlobal_config[2] = 300;



 pGlobal_config[3] = 143;
 pGlobal_config[4] = 160;


 pGlobal_config[18] = 50000;
 pGlobal_config[37] = 1000;

 pGlobal_config[19] = 2000;
 pGlobal_config[45] = 5000;



 pGlobal_config[9] = 1000;
 pGlobal_config[10] = 6250;


 pGlobal_config[23] = 115;
 ...
 _symrom_g_smartWindowSize = pGlobal_config[19] ;


 if(_symrom_g_system_clk==SYS_CLK_XTAL_16M)
 {
  pGlobal_config[31] = 18;
 }
 else if(_symrom_g_system_clk==SYS_CLK_DLL_48M)
 {
  pGlobal_config[31] = 10;
 }
 else if(_symrom_g_system_clk==SYS_CLK_DLL_64M)
 {
  pGlobal_config[31] = 8;
 ...
 pGlobal_config[39] = 56;

    pGlobal_config[44] = 0x11004000;

 pGlobal_config[40] = 1400*3;
 pGlobal_config[41] = 600;

 pGlobal_config[42] = 1400*3;
 pGlobal_config[43] = 2000;

 pGlobal_config[13] = 30;
 pGlobal_config[46] = 300;
 pGlobal_config[47] = 50;

 extern void osalInitTasks( void );
 extern pTaskEventHandlerFn tasksArr[];
 extern uint16 tasksCnt;
 extern uint16 *tasksEvents;
 _symrom_JUMP_FUNCTION_SET(1,(uint32_t)&osalInitTasks);
 _symrom_JUMP_FUNCTION_SET(2,(uint32_t)&tasksArr);
 _symrom_JUMP_FUNCTION_SET(3 ,(uint32_t)&tasksCnt);
 _symrom_JUMP_FUNCTION_SET(4,(uint32_t)&tasksEvents);
 ...
 _symrom_JUMP_FUNCTION_SET(69 , (uint32_t)&rf_phy_ini1);
 _symrom_JUMP_FUNCTION_SET(73 , (uint32_t)&rf_phy_change_cfg1);
 _symrom_JUMP_FUNCTION_SET(33 , (uint32_t)&ll_generateTxBuffer1);

 _symrom_JUMP_FUNCTION_SET(33 , (uint32_t)&ll_generateTxBuffer1);

     _symrom_JUMP_FUNCTION_SET(70 , (uint32_t)&wakeup_init1);
...
{
    pGlobal_config[44] = macAddr;
}

extern void _symrom_ble_main(void);
