# 1 "../src_patch/rf_phy_driver.c"
# 1 "C:\\Dfile\\test\\jack_test_sripts_F1\\lib\\generate_lib\\ck802_proj//"
# 1 "<built-in>"
#define __STDC__ 1
...
# 49 "../src_patch/rf_phy_driver.c" 2
...
# 11 "../../../components/driver/include/clock.h" 2
...
# 86 "../../../components/driver/include/log.h"
#define AT_LOG(...) 
#define LOG_DEBUG(...) 
#define LOG(...) 
#define LOG_INIT() 
...
# 50 "../src_patch/rf_phy_driver.c" 2


# 1 "../../../components/driver/include/timer.h" 1
# 53 "../src_patch/rf_phy_driver.c" 2
# 1 "../../../components/ble/controller/ll_hw_drv.h" 1
# 54 "../src_patch/rf_phy_driver.c" 2
...
# 1 "../../../components/driver/include/log.h" 1
# 90 "../src_patch/rf_phy_driver.c" 2
...
#define DTM_ADD_IDX(a,b) {(a==b)? a=0:a++;}
    static unsigned char urx_buf[32];
    static volatile uint32_t uart_rx_wIdx=0,uart_rx_rIdx=0;
# 113 "../src_patch/rf_phy_driver.c"
void rf_phy_change_cfg1(uint8_t pktFmt)
{
    _symrom_rf_phy_change_cfg0(pktFmt);
    {*(volatile uint32_t *)(0x400300d8) = (0x04890000);__asm volatile("nop");};
    if(pktFmt==1)
    {
        {(*(volatile unsigned int *)(0x400300d8)=(unsigned int)(((*(volatile unsigned int *)(0x400300d8))& (~((((unsigned int)1<<((20)-(18)+1))-1)<<(18))))| ((unsigned int)(0x01)<<(18))));__asm volatile("nop");};
    }
    else
    {
        {(*(volatile unsigned int *)(0x400300d8)=(unsigned int)(((*(volatile unsigned int *)(0x400300d8))& (~((((unsigned int)1<<((20)-(18)+1))-1)<<(18))))| ((unsigned int)(0x02)<<(18))));__asm volatile("nop");};
...
void rf_phy_set_txPower (uint8 txPower)
{
    if(0x3f==txPower)
    {
        ...
        {(*(volatile unsigned int *)(0x400300dc)=(unsigned int)(((*(volatile unsigned int *)(0x400300dc))& (~((((unsigned int)1<<((9)-(7)+1))-1)<<(7))))| ((unsigned int)((0x03&(1)))<<(7))));__asm volatile("nop");};
    }
    else
    {
        ...
        {(*(volatile unsigned int *)(0x400300dc)=(unsigned int)(((*(volatile unsigned int *)(0x400300dc))& (~((((unsigned int)1<<((9)-(7)+1))-1)<<(7))))| ((unsigned int)((0x03&(0)))<<(7))));__asm volatile("nop");};
    }

    {*(volatile uint32_t *)(0x400300b8) = ((*(volatile uint32_t *)(0x400300b8)&0x0fff) | ((txPower&0x1f)<<12));__asm volatile("nop");};
}
void rf_phy_ini1(void)
{
    _symrom_rf_phy_ini();
    ...
    {*(volatile uint32_t *)(0x40031074) = (0x00800080);__asm volatile("nop");};

    {(*(volatile unsigned int *)(0x400300dc)=(unsigned int)(((*(volatile unsigned int *)(0x400300dc))& (~((((unsigned int)1<<((6)-(3)+1))-1)<<(3))))| ((unsigned int)(0)<<(3))));__asm volatile("nop");};
    {(*(volatile unsigned int *)(0x400300cc)=(unsigned int)(((*(volatile unsigned int *)(0x400300cc))& (~((((unsigned int)1<<((11)-(6)+1))-1)<<(6))))| ((unsigned int)(0)<<(6))));__asm volatile("nop");};
}
# 200 "../src_patch/rf_phy_driver.c"
void DTM_UART_IRQHandler(void)
{
    uint8_t IRQ_ID= (((AP_UART_TypeDef *) ((0x40000000UL) + 0x4000))->IIR & 0x0f);

    switch (IRQ_ID)
    {
    case RDA_IRQ:
    case TIMEOUT_IRQ:
        while(((AP_UART_TypeDef *) ((0x40000000UL) + 0x4000)) ->LSR & 0x1)
        {
            urx_buf[uart_rx_wIdx]=(((AP_UART_TypeDef *) ((0x40000000UL) + 0x4000))->RBR & 0xff);
            {(uart_rx_wIdx==(32 -1))? uart_rx_wIdx=0:uart_rx_wIdx++;};
        }

        break;

    case BUSY_IRQ:
        ((AP_UART_TypeDef *) ((0x40000000UL) + 0x4000)) -> USR;
        break;
    }
}
# 247 "../src_patch/rf_phy_driver.c"
void rf_phy_direct_test(void)
{
    int dtmState = 0;
    uint32_t deltTick = 0;
    ...
    _symrom_JUMP_FUNCTION_SET(113,(uint32_t)&DTM_UART_IRQHandler);
    if((0x01|0x02|(0x04|0x08))&0x01)
        _symrom_log_printf("=== SUPPORT BLE 1M  ===\n");

    if((0x01|0x02|(0x04|0x08))&0x02)
        _symrom_log_printf("=== SUPPORT BLE 2M  ===\n");


    *(volatile int*) 0xe000e100 = 0x800;
    *(volatile unsigned int*) 0x40004004 |= 0x01;



    _symrom_set_timer(((AP_TIM_TypeDef *) ((0x40000000UL) + 0x1028)),(0x003fffff));
    ...
    while(1)
    {
        if( dtmState == 0)
        {
            if((uart_rx_wIdx)>=2)
            {
                _symrom_g_rfPhyDtmCmd[0]=urx_buf[0];
                _symrom_g_rfPhyDtmCmd[1]=urx_buf[1];
                dtmState = 1;
                {uart_rx_wIdx=0;};
            }
        ...
        {
            rf_phy_dtm_evt_send(_symrom_g_dtmModeType);
            dtmState = 3;
        ...
        else if(dtmState == 3)
        {
            ...
            while((uart_rx_wIdx)<2)
            {

                if(_symrom_g_dtmModeType == 2)
                {



                    _symrom_ll_hw_set_trx_settle (100, 8, 90);

                    currTick = _symrom_read_current_fine_time();
                    deltTick = ( (currTick>=_symrom_g_dtmTick) ? currTick-_symrom_g_dtmTick : (0x003fffff)-_symrom_g_dtmTick+currTick );

                    if( (_symrom_ll_hw_get_irq_status() & 0x0001)
                            && _symrom_g_rfPhyPktFmt == 0
                            && zgb_pkt_flg==0)
                    {


                        _symrom_ll_hw_clr_irq();
                        zgb_pkt_flg=1;
                        {*(volatile uint32_t *)(0x4003105c) = (0x00000000);__asm volatile("nop");};
                    }

                    if(deltTick>=_symrom_g_dtmPktIntv)
                    {
                        if(!(_symrom_g_rfPhyPktFmt==0))
                        {
                            _symrom_ll_hw_rst_tfifo();
                            _symrom_ll_hw_trigger();
# 357 "../src_patch/rf_phy_driver.c"
                        }
                        else
                        {
                            _symrom_ll_hw_trigger();
...
# 408 "../src_patch/rf_phy_driver.c"
                        _symrom_g_dtmTick = _symrom_g_dtmTick+_symrom_g_dtmPktIntv;

                        if(_symrom_g_dtmTick>(0x003fffff))
                            _symrom_g_dtmTick = _symrom_g_dtmTick-(0x003fffff);
                    }
                }
                else if( _symrom_g_dtmModeType == 8
                           || _symrom_g_dtmModeType == 138)
                {

                    if(_symrom_g_dtmManualConfig&0x08)
                    {
                        {(*(volatile unsigned int *)(0x40030050)=(unsigned int)(((*(volatile unsigned int *)(0x40030050))& (~((((unsigned int)1<<((4)-(0)+1))-1)<<(0))))| ((unsigned int)(0x10)<<(0))));__asm volatile("nop");};
                    }

                    {(*(volatile unsigned int *)(0x4000f018)=(unsigned int)(((*(volatile unsigned int *)(0x4000f018))& (~((((unsigned int)1<<((14)-(9)+1))-1)<<(9))))| ((unsigned int)(0x3f)<<(9))));__asm volatile("nop");};

                    if(_symrom_ll_hw_get_irq_status()&0x0001)
                    {
                        if(_symrom_ll_hw_get_irq_status()&0x0400)
                        {
                            _symrom_g_dtmPktCount++;
                            _symrom_rf_phy_get_pktFoot(&rssi,&foff,&carrSens);
                        }
                        else if(_symrom_ll_hw_get_irq_status()&0x0002)
                        {
                            _symrom_g_dtmRxCrcNum++;
                        }
                        else if(_symrom_ll_hw_get_irq_status()&0x0004)
                        {
                            _symrom_g_dtmRxTONum++;
                        }
                        else
                        {

                            _symrom_g_dtmPktCount= (_symrom_g_dtmPktCount==65535) ? 0 : _symrom_g_dtmPktCount;
                        }

                        if( _symrom_g_dtmModeType == 138)
                        {
                            currTick = _symrom_read_current_fine_time();
                            deltTick = ( (currTick>=_symrom_g_dtmTick) ? currTick-_symrom_g_dtmTick : (0x003fffff)-_symrom_g_dtmTick+currTick );

                            if(deltTick>_symrom_g_dtmPerAutoIntv)
                            {
                                rf_phy_dtm_evt_send(10);
                                _symrom_WaitUs(500);
                                rf_phy_dtm_evt_send(130);
                                _symrom_WaitUs(500);
                                rf_phy_dtm_evt_send(134);
                                _symrom_WaitUs(500);
                                rf_phy_dtm_evt_send(136);
                                _symrom_WaitUs(500);
                                _symrom_g_dtmPktCount = 0;
                                _symrom_g_dtmRxCrcNum = 0;
                                _symrom_g_dtmRxTONum = 0;
                                _symrom_g_dtmTick = _symrom_read_current_fine_time();
                            }
                        }
                        ...
                        if(_symrom_g_dtmPktCount>0)
                        {
                            _symrom_g_dtmRssi = (_symrom_g_dtmPktCount==1) ? rssi :((_symrom_g_dtmRssi+rssi) >>1);
                            _symrom_g_dtmFoff = (_symrom_g_dtmPktCount==1) ? foff :((_symrom_g_dtmFoff+foff) >>1);
                            _symrom_g_dtmCarrSens = (_symrom_g_dtmPktCount==1) ? carrSens :((_symrom_g_dtmCarrSens+carrSens) >>1);
                        }
                    }
                }
            }
...
# 505 "../src_patch/rf_phy_driver.c"
void rf_phy_dtm_cmd_parse(void)
{
    _symrom_g_dtmCmd = ((_symrom_g_rfPhyDtmCmd[0] & 0xc0)>>6);


    if(_symrom_g_dtmCmd == 0 || _symrom_g_dtmCmd==3 )
    {
        _symrom_g_dtmCtrl = ((_symrom_g_rfPhyDtmCmd[0] & 0x3f) );
        _symrom_g_dtmPara = ((_symrom_g_rfPhyDtmCmd[1] & 0xfc)>>2);


        if(_symrom_g_dtmCmd==0)
        {
            if(_symrom_g_dtmCtrl==0 && _symrom_g_dtmPara==0)
            {
                _symrom_g_dtmModeType = 0;
            }
            else if(_symrom_g_dtmCtrl==1)
            {
                _symrom_g_dtmModeType = 12;
                _symrom_g_dtmExtLen = (_symrom_g_dtmPara&0x3);
            }
            else if(_symrom_g_dtmCtrl==2 && _symrom_g_dtmPara ==0x01)
            {
                _symrom_g_dtmModeType = 16;
                _symrom_g_rfPhyPktFmt = 1;
            }
            else if(_symrom_g_dtmCtrl==2 && _symrom_g_dtmPara ==0x02)
            {
                _symrom_g_dtmModeType = 18;
                _symrom_g_rfPhyPktFmt = (0x02 & (0x01|0x02|(0x04|0x08)))
                                ?2:1;
            }
            else if(_symrom_g_dtmCtrl==2 && _symrom_g_dtmPara ==0x03)
            {
                _symrom_g_dtmModeType = 22;
                _symrom_g_rfPhyPktFmt = (0x08 & (0x01|0x02|(0x04|0x08)))
                                ?4:1;
            }
            else if(_symrom_g_dtmCtrl==2 && _symrom_g_dtmPara ==0x04)
            {
                _symrom_g_dtmModeType = 20;
                _symrom_g_rfPhyPktFmt = (0x04 & (0x01|0x02|(0x04|0x08)))
                                ?3:1;
            }
            else if(_symrom_g_dtmCtrl==2 && _symrom_g_dtmPara ==0x20)
            {
                _symrom_g_dtmModeType = 24;
                _symrom_g_rfPhyPktFmt = (0x10 & (0x01|0x02|(0x04|0x08)))
                                ?0:1;
            }
            else if(_symrom_g_dtmCtrl==3 && _symrom_g_dtmPara ==0x00)
            {
                _symrom_g_dtmModeType = 32;
            }
            else if(_symrom_g_dtmCtrl==3 && _symrom_g_dtmPara ==0x01)
            {
                _symrom_g_dtmModeType = 34;
            }
            else if(_symrom_g_dtmCtrl==4 && _symrom_g_dtmPara ==0x00)
            {
                _symrom_g_dtmModeType = 36;
            }
            else if(_symrom_g_dtmCtrl==5 && _symrom_g_dtmPara ==0x00)
            {
                _symrom_g_dtmModeType = 38;
            }
            else if(_symrom_g_dtmCtrl==5 && _symrom_g_dtmPara ==0x01)
            {
                _symrom_g_dtmModeType = 40;
            }
            else if(_symrom_g_dtmCtrl==5 && _symrom_g_dtmPara ==0x02)
            {
                _symrom_g_dtmModeType = 42;
            }
            else if(_symrom_g_dtmCtrl==5 && _symrom_g_dtmPara ==0x03)
            {
                _symrom_g_dtmModeType = 44;

            }
            else if(_symrom_g_dtmCtrl==0x36)
            {
                _symrom_g_dtmModeType = 114;
                _symrom_g_dtmAccessCode = (_symrom_g_dtmAccessCode&(0xffffff00))|_symrom_g_rfPhyDtmCmd[1];

            }
            else if(_symrom_g_dtmCtrl==0x37)
            {
                _symrom_g_dtmModeType = 116;
                _symrom_g_dtmAccessCode = (_symrom_g_dtmAccessCode&(0xffff00ff))|(_symrom_g_rfPhyDtmCmd[1]<<8);

            }
            else if(_symrom_g_dtmCtrl==0x38)
            {
                _symrom_g_dtmModeType = 118;
                _symrom_g_dtmAccessCode = (_symrom_g_dtmAccessCode&(0xff00ffff))|(_symrom_g_rfPhyDtmCmd[1]<<16);

            }
            else if(_symrom_g_dtmCtrl==0x39)
            {
                _symrom_g_dtmModeType = 120;
                _symrom_g_dtmAccessCode = (_symrom_g_dtmAccessCode&(0x00ffffff))|(_symrom_g_rfPhyDtmCmd[1]<<24);

            }
            else if(_symrom_g_dtmCtrl==0x3a)
            {
                _symrom_g_dtmModeType = 122;
                if(_symrom_g_dtmManualConfig & 0x01)
                {
                    _symrom_g_rfPhyFreqOffSet = (_symrom_g_dtmPara-10)*5;
                }

            }
            else if(_symrom_g_dtmCtrl==0x3b)
            {
                _symrom_g_dtmModeType = 124;

                if(_symrom_g_dtmPara==0)
                {
                    _symrom_g_dtmTpCalEnable = 1;
                }
                else
                {
                    _symrom_g_dtmTpCalEnable = 0;
                    _symrom_g_rfPhyTpCal0 = (_symrom_g_dtmPara)<<1;
                    {*(volatile uint32_t *)(0x40030094) = (0x00001000+_symrom_g_rfPhyTpCal0);__asm volatile("nop");};
                }


            }
            else if(_symrom_g_dtmCtrl==0x3c)
            {
                _symrom_g_dtmModeType = 126;

                if(_symrom_g_dtmManualConfig & 0x04)
                {
                    {(*(volatile unsigned int *)(0x4000f0bc)=(unsigned int)(((*(volatile unsigned int *)(0x4000f0bc))& (~((((unsigned int)1<<((4)-(0)+1))-1)<<(0))))| ((unsigned int)((0x1f&(_symrom_g_dtmPara)))<<(0))));__asm volatile("nop");};
                }


            }
            else if(_symrom_g_dtmCtrl==0x3d)
            {
                _symrom_g_dtmModeType = 128;

                if(_symrom_g_dtmManualConfig & 0x02)
                {
                    _symrom_g_dtmTxPower = _symrom_g_dtmPara;
                }


            }
            else if(_symrom_g_dtmCtrl==0x3e)
            {
                _symrom_g_dtmModeType = 4;
                _symrom_g_dtmFreq = _symrom_g_dtmPara;

            }
            else if(_symrom_g_dtmCtrl==0x3f)
            {
                _symrom_g_dtmModeType = 6;
                _symrom_g_dtmFreq = _symrom_g_dtmPara;
            }
            else
            {
                _symrom_g_dtmModeType = 254;
            }
        ...
        else
        {
            if(_symrom_g_dtmCtrl==0 && _symrom_g_dtmPara==0)
            {
                _symrom_g_dtmModeType = 10;

            }
            else if(_symrom_g_dtmCtrl==0x3f && _symrom_g_dtmPara==0)
            {
                _symrom_g_dtmModeType = 130;

            }
            else if(_symrom_g_dtmCtrl==0x3f && _symrom_g_dtmPara==1)
            {
                _symrom_g_dtmModeType = 132;

            }
            else if(_symrom_g_dtmCtrl==0x3f && _symrom_g_dtmPara==2)
            {
                _symrom_g_dtmModeType = 134;

            }
            else if(_symrom_g_dtmCtrl==0x3f && _symrom_g_dtmPara==3)
            {
                _symrom_g_dtmModeType = 136;

            }
            else if(_symrom_g_dtmCtrl==0x3f && _symrom_g_dtmPara==4)
            {
                _symrom_g_dtmModeType = 138;
            }
            else
    ...
    else if( _symrom_g_dtmCmd == 1 || _symrom_g_dtmCmd == 2)
    {
        _symrom_g_dtmFreq = ((_symrom_g_rfPhyDtmCmd[0] & 0x3f) );
        _symrom_g_dtmLength = ((_symrom_g_rfPhyDtmCmd[1] & 0xfc)>>2);
        _symrom_g_dtmPKT = ((_symrom_g_rfPhyDtmCmd[1] & 0x03) );

        if(_symrom_g_dtmPktCount==3 && (_symrom_g_rfPhyPktFmt==4 || _symrom_g_rfPhyPktFmt==3 ))
        {
            _symrom_g_dtmPKT = 4;
        }

        _symrom_g_dtmModeType = (_symrom_g_dtmCmd ==1 )? 8 : 2;
    }
}
# 749 "../src_patch/rf_phy_driver.c"
void rf_phy_dtm_evt_send(uint8_t dtmType )
{

    _symrom_g_rfPhyDtmEvt[0] = 0;
    _symrom_g_rfPhyDtmEvt[1] = 0;

    if(dtmType==254)
    {
        _symrom_g_dtmEvt = 0;
        _symrom_g_dtmStatus = 1;
    }
    else
    {
        if(dtmType == 10)
        {
            _symrom_g_dtmEvt = 0x80;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_dtmPktCount;
        }
        else if(dtmType == 130)
        {
            _symrom_g_dtmEvt = 0x80;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_dtmFoff;
        }
        else if(dtmType == 132)
        {
            _symrom_g_dtmEvt = 0x80;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_rfPhyTpCal0;
        }
        else if(dtmType == 134 )
        {
            _symrom_g_dtmEvt = 0x80;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_dtmRssi;
        }
        else if(dtmType == 136)
        {
            _symrom_g_dtmEvt = 0x80;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_dtmCarrSens;
        }
        else if(dtmType == 36)
        {
            _symrom_g_dtmEvt = 0x00;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = 0x0c;
        ...
        else if(dtmType == 38)
        {
            _symrom_g_dtmEvt = 0x00;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = 27<<1;
        }
        else if(dtmType == 40)
        {
            _symrom_g_dtmEvt = 0x00;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = 1352<<1;
        }
        else if(dtmType == 42)
        {
            _symrom_g_dtmEvt = 0x00;
        ...
        else
        {
            _symrom_g_dtmEvt = 0;
            _symrom_g_dtmStatus = 0;
            _symrom_g_dtmRsp = _symrom_g_dtmModeType;
        }
    }

    _symrom_g_rfPhyDtmEvt[0] = _symrom_g_rfPhyDtmEvt[0] | _symrom_g_dtmEvt | (_symrom_g_dtmRsp >> 8);
    _symrom_g_rfPhyDtmEvt[1] = _symrom_g_rfPhyDtmEvt[1] | _symrom_g_dtmStatus | (_symrom_g_dtmRsp & 0xff);
    _symrom_hal_uart_send_byte(UART0,_symrom_g_rfPhyDtmEvt[0]);
    _symrom_hal_uart_send_byte(UART0,_symrom_g_rfPhyDtmEvt[1]);
}
# 860 "../src_patch/rf_phy_driver.c"
void rf_phy_dtm_trigged(void)
{

    _symrom_g_dtmLength = ((0x03&_symrom_g_dtmExtLen)<<6)|(0x3f&_symrom_g_dtmLength);
    int pktLenUs =0;
    uint8_t rfChn = 2+(_symrom_g_dtmFreq<<1);
    uint8_t preambleLen = 1;
# 875 "../src_patch/rf_phy_driver.c"
    if( _symrom_g_rfPhyPktFmt==1)
    {
        pktLenUs = (_symrom_g_dtmLength+1+4+2+3)<<3;
    }
    else if( _symrom_g_rfPhyPktFmt==2)
    {
        pktLenUs = (_symrom_g_dtmLength+2+4+2+3)<<2;
    }
    else if( _symrom_g_rfPhyPktFmt==3)
    {
        pktLenUs = 80+296+((_symrom_g_dtmLength+2+3)<<4)+6;
    }
    else if( _symrom_g_rfPhyPktFmt==4)
    {
        pktLenUs = 80+296+((_symrom_g_dtmLength+2+3)<<6)+24;
    }
    else
    {
        pktLenUs = (_symrom_g_dtmLength+4+1+1)<<5;
        rfChn = 5+_symrom_g_dtmFreq*5;
    }

    _symrom_g_dtmPktIntv = (((pktLenUs+249)+624)/625)*625;

    if(_symrom_g_dtmModeType == 138)
    {
        _symrom_g_dtmPerAutoIntv = _symrom_g_dtmPktIntv*1000;
        _symrom_g_dtmTick = _symrom_read_current_fine_time();
    }

    {*(volatile uint32_t *)(0x40030040) = (0x00030000);__asm volatile("nop");};
    {*(volatile uint32_t *)(0x400300a0) = (0x0000000e);__asm volatile("nop");};
    {*(volatile uint32_t *)(0x400300a4) = (0x00000100);__asm volatile("nop");};
    {*(volatile uint32_t *)(0x400300a0) = (0x00000000);__asm volatile("nop");};
    {*(volatile uint32_t *)(0x4003008c) = (0x00104040);__asm volatile("nop");};

    if(_symrom_g_dtmModeType == 0)
    {
        _symrom_g_dtmPktCount = 0;
        _symrom_g_dtmRsp = 0;
        _symrom_g_dtmExtLen = 0;
        _symrom_g_dtmLength = 0;
        _symrom_g_dtmPktIntv = 0;
        _symrom_g_dtmTxPower = _symrom_g_rfPhyTxPower;
        _symrom_g_dtmFoff = 0;
        _symrom_g_dtmRssi = 0;
        _symrom_g_dtmCarrSens = 0;
        _symrom_g_rfPhyPktFmt = 1;
        _symrom_g_dtmPKT = 0;
        {(*(volatile unsigned int *)(0x4000f014)=(unsigned int)(((*(volatile unsigned int *)(0x4000f014))& (~((((unsigned int)1<<((18)-(15)+1))-1)<<(15))))| ((unsigned int)((0x0f&(0x0a)))<<(15))));__asm volatile("nop");};
    }
    else if( _symrom_g_dtmModeType == 2
               || _symrom_g_dtmModeType == 4
               || _symrom_g_dtmModeType == 6 )
    {

        if(_symrom_g_dtmTpCalEnable)
        {
            _symrom_rf_phy_ana_cfg();

            _symrom_rf_tpCal_cfg_avg(rfChn,4);
        }


        rf_phy_ini1();
        rf_phy_set_txPower(_symrom_g_dtmTxPower);
        _symrom_ll_hw_set_timing(_symrom_g_rfPhyPktFmt);


        if(_symrom_g_rfPhyFreqOffSet>=0)
  {
            {*(volatile uint32_t *)(0x400300b4) = (rfChn+(_symrom_g_rfPhyFreqOffSet<<8));__asm volatile("nop");};
        }
  else
  {
            {*(volatile uint32_t *)(0x400300b4) = (rfChn-1+((255+_symrom_g_rfPhyFreqOffSet)<<8));__asm volatile("nop");};
  }
        if(!(_symrom_g_rfPhyPktFmt==0))
        {
            {*(volatile uint32_t *)(0x40030048) = (0x00555555);__asm volatile("nop");};
            {*(volatile uint32_t *)(0x4003004c) = (_symrom_g_dtmAccessCode);__asm volatile("nop");};
            {*(volatile uint32_t *)(0x40030040) = (0x00030010);__asm volatile("nop");};
        }



        {*(volatile uint32_t *)(0x40030044) = (0xffffffff);__asm volatile("nop");};

        if(_symrom_g_dtmModeType == 6)
        {
            {*(volatile uint32_t *)(0x400300a4) = (0x00000100);__asm volatile("nop");};
        }
        else
        {
            {*(volatile uint32_t *)(0x400300a4) = (0x00000140);__asm volatile("nop");};
        }
...
# 997 "../src_patch/rf_phy_driver.c"
        ...
        else
        {
            if(!(_symrom_g_rfPhyPktFmt==0))
            {
                ...
                {*(volatile uint32_t *)(0x40030040) = (0x00030010|(preambleLen<<5));__asm volatile("nop");};
            }
            else
            {

                {*(volatile uint32_t *)(0x40030040) = (0x000b0013|(preambleLen<<5));__asm volatile("nop");};
                {*(volatile uint32_t *)(0x40030000) = (0x78068002);__asm volatile("nop");};
            }
        }


        _symrom_ll_hw_set_irq(0);
        _symrom_ll_hw_set_stx();
        _symrom_ll_hw_clr_irq();
    ...
    {
        _symrom_rf_phy_ana_cfg();
        _symrom_rf_rxDcoc_cfg( 2, 1,&_symrom_g_rfPhyRxDcIQ);

        rf_phy_ini1();
        _symrom_ll_hw_set_timing(_symrom_g_rfPhyPktFmt);
        {*(volatile uint32_t *)(0x40031024) = (_symrom_g_dtmPktIntv-100);__asm volatile("nop");};
        {*(volatile uint32_t *)(0x40031028) = (_symrom_g_dtmPktIntv-100);__asm volatile("nop");};

        if((rfChn&0x0f)==0)
        {
            {*(volatile uint32_t *)(0x400300b4) = (rfChn);__asm volatile("nop");};
            {(*(volatile unsigned int *)(0x4003011c)=(unsigned int)(((*(volatile unsigned int *)(0x4003011c))& (~((((unsigned int)1<<((11)-(8)+1))-1)<<(8))))| ((unsigned int)(3)<<(8))));__asm volatile("nop");};
        }
        else
        {
            {(*(volatile unsigned int *)(0x4003011c)=(unsigned int)(((*(volatile unsigned int *)(0x4003011c))& (~((((unsigned int)1<<((11)-(8)+1))-1)<<(8))))| ((unsigned int)(0)<<(8))));__asm volatile("nop");};

            if(_symrom_g_rfPhyFreqOffSet>=0)
   {
                {*(volatile uint32_t *)(0x400300b4) = (rfChn+(_symrom_g_rfPhyFreqOffSet<<16));__asm volatile("nop");};
            }
   else
   {
                {*(volatile uint32_t *)(0x400300b4) = (rfChn-1+((255+_symrom_g_rfPhyFreqOffSet)<<16));__asm volatile("nop");};
   }
        ...
        {*(volatile uint32_t *)(0x4003000c) = ((_symrom_g_dtmLength+3)>255 ? 255:( _symrom_g_dtmLength<40 ? 40 : _symrom_g_dtmLength+3));__asm volatile("nop");};

        _symrom_ll_hw_set_irq(0);
        _symrom_ll_hw_set_srx();
        _symrom_ll_hw_clr_irq();
        _symrom_ll_hw_trigger();
    }
}
void gen_pn_prbs9(uint16_t seed, int length,uint8_t* pnOut)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        _symrom_bit_to_byte(bitOut, pnOut + i);
    }
}
void rf_phy_dtm_ble_pkt_gen(void)
{
    uint8_t tmp[256];
    tmp[1] = _symrom_g_dtmLength;
    tmp[0] = _symrom_g_dtmPKT;
    uint8_t pld=0x00;

    if(_symrom_g_dtmPKT==1)
    {
        pld = 0x0f;
    }
    else if(_symrom_g_dtmPKT==2)
    {
        pld=0x55;
    }
    else if(_symrom_g_dtmPKT==3)
    {
        pld=0xff;
        tmp[0]=4;
    }

    if(_symrom_g_dtmPKT==0)
    {
        gen_pn_prbs9(0x01ff,_symrom_g_dtmLength,&(tmp[2]));
    }
    else
    {
        for(uint8_t i=0; i<_symrom_g_dtmLength; i++)
            tmp[2+i] =pld;
    }
    ...
    if(_symrom_g_rfPhyPktFmt==1)
    {
        {(*(volatile unsigned int *)(0x40030040)=(unsigned int)(((*(volatile unsigned int *)(0x40030040))& (~((((unsigned int)1<<((7)-(5)+1))-1)<<(5))))| ((unsigned int)(((8)>>3))<<(5))));__asm volatile("nop");};
    }
    else if(_symrom_g_rfPhyPktFmt == 2)
    {
        {(*(volatile unsigned int *)(0x40030040)=(unsigned int)(((*(volatile unsigned int *)(0x40030040))& (~((((unsigned int)1<<((7)-(5)+1))-1)<<(5))))| ((unsigned int)(((8)>>2))<<(5))));__asm volatile("nop");};
    }

    _symrom_ll_hw_write_tfifo(tmp,_symrom_g_dtmLength+2);
}
